...
3. 若在中國大陸使用本工具，您可能需要更改預設 npm registry 至[淘宝 npm 镜像](https://developer.aliyun.com/mirror/NPM)，以加快下載速度：<details><summary>（點擊本處可獲得更詳細的說明）</summary>
   * 請將下列文字加入 `%USERPROFILE%\.npmrc` (Windows) 或 `$HOME/.npmrc` (Linux)：可參考[解决 Node.js 由于网络原因导致模块安装和下载失败的方法](https://github.com/gee1k/notes/issues/23)
      ```
   ...
   * 若您無法找到 [.npmrc](https://docs.npmjs.com/misc/config) 設定檔，下面提供暫時性的解決方法：在命令列介面下執行
      ```bash
