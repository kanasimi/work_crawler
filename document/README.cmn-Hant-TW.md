...
| [禁漫天堂](https://18comic.vip/) | 18comic.js | 從 2021/3/12 開始將圖片分割、亂序排列。必須解開壓縮檔後，利用其中的 index.html 來瀏覽。 |
...
1. 先安裝 [node.js](https://nodejs.org/) 與 [7-Zip](https://www.7-zip.org/) 18.01 以上的版本。<!-- 下載小說須先安裝 [7-Zip](https://en.wikipedia.org/wiki/7-Zip) 以製作 .epub 電子書。 -->**請將程式安裝於預設路徑下，否則可能出現錯誤！**（例如 Windows 系統下，安裝 7-Zip 於 `C:\Program Files\`）已經安裝過的可以跳過。Node.js 用以執行本工具。7-Zip 在更新本工具，或者打包漫畫章節，以及製作電子書時使用。
