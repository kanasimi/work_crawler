...
3. 若在中国大陆使用本工具，您可能需要更改预设 npm registry 至[淘宝 npm 镜像](https://developer.aliyun.com/mirror/NPM)，以加快下载速度：<details><summary>（单击本处可获得更详细的说明）</summary>
   * 请将下列文字加入 `%USERPROFILE%\.npmrc` (Windows) 或 `[$HOME/.npmrc](https://docs.npmjs.com/misc/config)` (Linux)：可参考[解决 Node.js 由于网络原因导致模块安装和下载失败的方法](https://github.com/gee1k/notes/issues/23)
      ```
   ...
   * 若您无法找到 `.npmrc` 设定档，下面提供暂时性的解决方法：在命令行界面下执行
      ```bash
